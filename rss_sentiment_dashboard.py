...
def classify_headline(headline: str):
    ...
    return label, reason, confidence, pos, neg, neu
...
def analyze(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    outputs = [classify_headline(h) for h in df["headline"].tolist()]
    out = df.copy()
    out["predicted"] = [x[0] for x in outputs]
    out["reasoning"] = [x[1] for x in outputs]
    out["confidence"] = [x[2] for x in outputs]
    out["score_positive"] = [x[3] for x in outputs]
    out["score_negative"] = [x[4] for x in outputs]
    out["score_neutral"] = [x[5] for x in outputs]
...
st.caption("More robust version: reduced neutral overbias, stronger directional rules, and safer decision thresholds")
...
view_cols = ["source", "headline", "predicted", "confidence", "reasoning", "published", "domain", "url"]
if show_scores:
